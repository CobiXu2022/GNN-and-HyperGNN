...
def load_data(data_path, noAgg=False):
    ...
    print("\nclass distribution:")
    print(df_class_feature['class'].value_counts())
    ...
    json_data = {
        "nodes": [
            ...
            for _, row in df_class_feature.iterrows()
        ],
        "links": [
            {
                "source": int(u),
            ...
            for u, v in zip(df_edges["txId1"].values, df_edges["txId2"].values)
        ]
    }

    # Save to JSON file
    with open(osp.join(data_path, 'graph_data.json'), 'w') as json_file:
